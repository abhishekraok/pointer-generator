...
def set_tf_flags_defalts(tf_app_flags):
  # Console article input
  tf_app_flags.DEFINE_string('input_article', '', 'To summarize a single article given in command line, '
                                                  'this is the input article.')
  # Where to find data
  tf_app_flags.DEFINE_string('data_path', '',
                             'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
  tf_app_flags.DEFINE_string('vocab_path', '', 'Path expression to text vocabulary file.')
  # Important settings
  tf_app_flags.DEFINE_string('mode', 'train', 'must be one of train/eval/decode')
  tf_app_flags.DEFINE_boolean('single_pass', False,
  ...
  tf_app_flags.DEFINE_string('log_root', '', 'Root directory for all logging.')
  tf_app_flags.DEFINE_string('exp_name', '',
                             'Name for experiment. Logs will be saved in a directory with this name, under log_root.')
  tf_app_flags.DEFINE_string('device', '/gpu:0', 'Whether to use gpu or cpu')
  # Hyperparameters
  tf_app_flags.DEFINE_integer('hidden_dim', 256, 'dimension of RNN hidden states')
  tf_app_flags.DEFINE_integer('emb_dim', 128, 'dimension of word embeddings')
  tf_app_flags.DEFINE_integer('batch_size', 16, 'minibatch size')
  tf_app_flags.DEFINE_integer('max_enc_steps', 400, 'max timesteps of encoder (max source text tokens)')
  tf_app_flags.DEFINE_integer('max_dec_steps', 100, 'max timesteps of decoder (max summary tokens)')
  tf_app_flags.DEFINE_integer('beam_size', 4, 'beam size for beam search decoding.')
  tf_app_flags.DEFINE_integer('min_dec_steps', 35,
                              'Minimum sequence length of generated summary. Applies only for beam search decoding mode')
  tf_app_flags.DEFINE_integer('vocab_size', 50000,
  ...
  tf_app_flags.DEFINE_integer('training_steps', 240000, 'Number of steps to train for during training.')
  tf_app_flags.DEFINE_float('lr', 0.15, 'learning rate')
  tf_app_flags.DEFINE_float('adagrad_init_acc', 0.1, 'initial accumulator value for Adagrad')
  tf_app_flags.DEFINE_float('rand_unif_init_mag', 0.02, 'magnitude for lstm cells random uniform inititalization')
  tf_app_flags.DEFINE_float('trunc_norm_init_std', 1e-4,
                            'std of trunc norm init, used for initializing everything else')
  tf_app_flags.DEFINE_float('max_grad_norm', 2.0, 'for gradient clipping')
  # Pointer-generator or baseline model
  tf_app_flags.DEFINE_boolean('pointer_gen', True,
                              'If True, use pointer-generator model. If False, use baseline model.')
  # Coverage hyperparameters
  tf_app_flags.DEFINE_boolean('coverage', False,
  ...
  tf_app_flags.DEFINE_float('cov_loss_wt', 1.0,
                            'Weight of coverage loss (lambda in the paper). If zero, then no incentive to minimize coverage loss.')
  tf_app_flags.DEFINE_boolean('convert_to_coverage_model', False,
...
def get_hyper_params_from_flag(flags):
